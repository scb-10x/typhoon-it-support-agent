...
# Available agents
AVAILABLE_AGENTS = [
    {"id": "agent_1", "name": "น้องเทค (IT Support)", "email": "techsupport@bluewave-tech.co.th"},
    ...
    {"id": "agent_4", "name": "พี่ซีเคียว (Security)", "email": "security@bluewave-tech.co.th"},
]
...
def _call_ticket_api(endpoint: str, method: str = "GET", data: Optional[Dict] = None) -> Dict:
    ...
    if "TICKET_API_TOKEN" in os.environ and os.getenv("TICKET_API_TOKEN") != "your-api-token":
        # Real API call
...
def _calculate_sla_targets(priority: str, created_at) -> Dict:
    ...
    return {
        "first_response_due": (created + timedelta(minutes=targets["first_response"])).isoformat(),
        "resolution_due": (created + timedelta(minutes=targets["resolution"])).isoformat(),
    }


def _check_sla_breach(created_at, priority: str, first_response_at = None, resolved_at = None) -> Dict:
    ...
    if first_response_at:
        first_response = datetime.fromisoformat(first_response_at) if isinstance(first_response_at, str) else first_response_at
    ...
    if resolved_at:
        resolved = datetime.fromisoformat(resolved_at) if isinstance(resolved_at, str) else resolved_at
...
@tool
def update_ticket_status(ticket_id: int, status: str, comment: Optional[str] = None) -> str:
...
def add_ticket_comment(ticket_id: int, comment: str, is_public: bool = True) -> str:
    ...
    ticket["history"].append({
        ...
        "actor": "IT Support Agent",
        "changes": {"comment": {"public": is_public, "preview": comment[:50] + "..." if len(comment) > 50 else comment}},
    })
...
def get_my_open_tickets(limit: int = 10) -> str:
    ...
    """
    open_tickets = [
        ticket for ticket in _get_tickets().values()

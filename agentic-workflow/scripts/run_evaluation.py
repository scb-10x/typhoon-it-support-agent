...
from typhoon_it_support.evaluation import TEST_CASES, evaluate_response, calculate_aggregate_metrics
...
def run_evaluation(
    ...
    for cat, stats in metrics['category_breakdown'].items():
        print(f"  {cat}:")
        print(f"    Pass rate: {stats['pass_rate']:.2%} ({stats['passed']}/{stats['total']})")
